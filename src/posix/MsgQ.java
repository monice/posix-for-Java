...
public class MsgQ extends IPC {
  ...
  public static class msqid_ds extends IPC.Perm {
    msqid_ds(int id) throws IPCException { // only we can create
      super(id);
      int rc = msgctl(id,IPC_STAT,this);
      if (rc < 0)
	throw new IPCException("msgctl",rc & 0x7fffffff);
    }
  ...
  /** Receive a message into a CPtr.  This can be convenient when the 
      ...
      the posix error code can be retrieved with the {@link posix.Errno}
  ...
  public int recv(CPtr msg,int type,int flag) throws IPCException {
	...
	if (rc < 0) {
	  rc = rc & 0x7FFFFFFF;
	  if (rc == Errno.EINTR) continue;
	  throw new IPCException("msgrcv("+id+")",rc);
	}
  ...
  public msqid_ds getStatus() throws IPCException {
    int id = this.id;
    if (id < 0) return null;
    return new msqid_ds(id);
  }

  /** Set the uid, gid, mode, qbytes for this IPC data structure.
      Only a process with appropriate privileges  can  raise  the  value  of
      msg_qbytes.
    @param st a current msqid_ds obtained from {@link #getStatus}
   */
  public void setPerm(Perm st) throws IPCException {
    if (!(st instanceof msqid_ds) || st.id != id)
      throw new IllegalArgumentException("Current msqid_ds required.");
    int rc = msgctl(id,IPC_SET,(msqid_ds)st);
    if (rc < 0)
      throw new IPCException("msgctl",rc & 0x7fffffff);
  }
